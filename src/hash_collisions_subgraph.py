...
def init_analysis_tables(connection: sqlite3.Connection) -> None:
    connection.executescript("""
        ...
        -- Index for phase-2 query: WHERE radius = ? AND is_collision = 1
...
def run_analyze_phase(connection: sqlite3.Connection, workers: int) -> None:
    print("\nPhase 1 — Color consistency analysis")
    print("=" * 60)
    ...
    for radius in RADII:
        ...
        print(
            f"  r={radius}: {n_singles:,} singles (no JSON needed) | {n_multi:,} multi-entry colors (workers fetch JSON)"
        )
